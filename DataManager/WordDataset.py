...
class WordDataset(data.Dataset):
    def __init__(self, img_dir, label_file_path, virtual_size=-1, inp_h=32, inp_w=128, transform=None):
        ...
        print(f"Initializing WordDataset with img_dir: {img_dir}, label_file_path: {label_file_path}")
        ...
        print(f"Loaded {len(label_file)} labels from {label_file_path}")
        ...
        print(f"Filtered {len(filtered_words)} valid words from {len(words)} total words")
    ...
    def __getitem__(self, idx):
        ...
        print(f"Loading image from path: {self.data[idx][0]}")
        ...
        print(f"Returning image and word: {self.data[idx][1]}")
